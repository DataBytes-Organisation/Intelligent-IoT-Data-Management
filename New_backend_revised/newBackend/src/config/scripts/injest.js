 ...
 * Usage: node src/scripts/injest.js <datasetName> <metric> <csvFilePath>
 */
const fs = require('fs');
...
async function ingestCsv({ datasetName, metric, filePath }) {
  ...
  await new Promise((resolve, reject) => {
    fs.createReadStream(abs)
      ...
      .on('data', (row) => {
        ...
        rows.push({ ts: row.ts, value: parseFloat(row.value), quality_flag: row.quality_flag || null });
      })
      .on('end', resolve)
      .on('error', reject);
  });
  ...
  // Insert in batches to avoid too many queries
  const batchSize = 500;
  for (let i=0;i<rows.length;i+=batchSize) {
    ...
    for (const r of batch) {
      ...
      params.push(datasetId, metric, r.ts, r.value, r.quality_flag);
    }
...
if (require.main === module) {
  ...
  if (args.length < 3) {
    console.error('Usage: node src/scripts/injest.js <datasetName> <metric> <csvFilePath>');
    process.exit(1);
  }
  ingestCsv({ datasetName: args[0], metric: args[1], filePath: args[2] })
